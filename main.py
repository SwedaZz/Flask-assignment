...
1.(3) Surasti, isvardinti ir pataisyti visas projekte rastas klaidas zemiau, uz bent 5 rastas ir pataisytas pilnas balas:
    a) Sign-up template neturi Last_name lauko, o duomenų bazė reikalauja šio lauko (last_name nullable=false).
    b) 'User' object has no attribute 'is_active' - prie User reikia pridėti UserMixin
    c) Sign-out neveikia, reikia pridėti logout_user() funkciją
    d) Sign-in dekoratorius neturi nurodytų metodų (reikia pridėti POST ir GET metodus)
    e) Sign-in funkcijoje buvo ieškomas ne "email", o "first_name" - sutvarkyta.
    f) "'form_in_html' is undefined" - prie Update_account_Information vietoje form_in_html nurodyta "form" - sutvarkyta.
    g) Update_account_Information nerodo esamų duomenų, metodą reikia pakeisti iš POST į GET
    h) Update_account_Information rodomi ir keičiami duomenys yra sumaišyti - vardą ir pavardę atvaizduoja/pakeičia
    į elektroninį paštą.
    i) Prie login_manager nepridėta konfiguracinė eilutė login_manager.init_app(app) - sutvarkyta.
...
def borrow_book(id):
    ...
    if not user.books:
        ...
        db.session.add(book)
        db.session.commit()
        flash('Book borrowed, please return in time', 'success')
        ...
        if not is_borrowed:
            ...
            print(is_borrowed)
            flash('Book is already taken by you', 'danger')
            return redirect(url_for('available_books'))
